...
# max the ball can go(left or right)
throw_angle_deg = 0.0
THROW_ANGLE_MIN = -15.0
THROW_ANGLE_MAX = +15.0
THROW_ANGLE_STEP = 1.5

# x,y,fallen,fall_t,fall_angle
pins = []
...
#alvi
def reset_ball() :
    # making the ball go back in restart
    global ball_x, ball_y, ball_vx, ball_vy, spin, charging, power_value, power_dir, state, throw_angle_deg
    ball_x = 0.0
    ball_y = BALL_START_Y
    ball_vx = 0.0
    ball_vy = 0.0
...
#alvi
def launch_ball():
    #measuring the ball speed angle spin
    global ball_vx, ball_vy, power_value, spin, state, current_throw_record
    min_speed, max_speed = 220.0, 520.0
    speed = min_speed + power_value * (max_speed - min_speed)

    angle = math.radians(throw_angle_deg)
    dirx = math.sin(angle)
    diry = math.cos(angle)

    ball_vx = dirx * speed + spin * 40.0
    ball_vy = diry * speed
    state = "roll"
...
#alvi
def start_charge_or_launch():
    #for controlling the power and launch with spacebar
    global state, charging
    if state == "aim":
        charging = True
        state = "charge"
    elif state == "charge":
...
def keyboard(key, x, y):
    ...
    #spinning of ball(right or left)
    if key == b'a':
        set_spin(-0.1)
    if key == b'd':
        set_spin(+0.1)

    #throw angle(right or left)
    if key == b'w':
        adjust_angle(+THROW_ANGLE_STEP)
    if key == b's':
        adjust_angle(-THROW_ANGLE_STEP)

    #for throwing ball
    if key == b' ':
        start_charge_or_launch()
...
#alvi
def set_spin(i) :
    #cannot chnge spin during ball moving
    global spin, max_spin
    if state in ("aim", "charge"):
        spin = max(-max_spin, min(+max_spin, spin + i))
#alvi
def adjust_angle(i):
    #cannot change angle during ball moving
    global throw_angle_deg
    if state in ("aim", "charge"):
        throw_angle_deg = max(THROW_ANGLE_MIN, min(THROW_ANGLE_MAX, throw_angle_deg + i))
#alvi
def special(key, x, y):
    #keeping the ball spinning while moving
    global ball_x, state
    if state in ("aim", "charge"):
        ...
        if key == GLUT_KEY_RIGHT:
            ball_x += 10.0
        half = (LANE_W*0.5) - BALL_RADIUS - 2.0
        ball_x = max(-half, min(half, ball_x))
...
def collide_ball_with_pins():
    global ball_vx, ball_vy, ball_x, ball_y
...
#all members
def update_roll():
    #checks the ball status and calls collision with pins
    global ball_x, ball_y, ball_vx, ball_vy, spin, state, current_throw_record
    ball_vx += spin * SPIN_CURVE * ball_vy
    ...
    #pins collision
    collide_ball_with_pins()

    current_throw_record.append({
        "ball_x": ball_x,
        "ball_y": ball_y,
        "pins": deepcopy(pins)
    })
    ...
    if ball_y > (PIN_DECK_Y + 120.0) or speed < 5.0:
        state = "settle"
